...
def load_files():
    ...
    injury_df.columns = [
        c.strip().lower().replace(" ", "_").replace(" ", "_") for c in injury_df.columns
    ]
    # Rename messy column names
    injury_df = injury_df.rename(
        columns={
...
def build_players_table(squad_df: pd.DataFrame, bio_df: pd.DataFrame) -> pd.DataFrame:
    ...
    players = squad_df.merge(
        bio_df, on="player_tm_id", how="left", suffixes=("_squad", "_bio")
    )

    # Resolve duplicate nationality columns — prefer bio nationality
    if "nationality_bio" in players.columns:
        players["nationality"] = players["nationality_bio"].fillna(
            players.get("nationality_squad", "")
        )
        players.drop(
            columns=["nationality_bio", "nationality_squad"],
            errors="ignore",
            inplace=True,
        )
    ...
    # Validate age range
    players.loc[
        players["age"].notna() & ((players["age"] < 15) | (players["age"] > 45)), "age"
    ] = None
...
def build_injuries_table(
    injury_df: pd.DataFrame, players: pd.DataFrame
) -> pd.DataFrame:
    ...
    inj["games_missed"] = pd.to_numeric(inj["games_missed"], errors="coerce").astype(
        "Int64"
    )
    ...
    if "player_tm_id" not in inj.columns or inj["player_tm_id"].isna().all():
        name_to_id = players.set_index("player_name_normalised")[
            "player_tm_id"
        ].to_dict()
    ...
    if unmatched:
        log.warning(
            f"  {unmatched} injury rows could not be matched to a player — dropping."
        )
    ...
    log.info(
        f"  Injuries table: {len(inj)} rows  |  Impact injuries: {inj['is_impact_injury'].sum()}"
    )
    return inj
